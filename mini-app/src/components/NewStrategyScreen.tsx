    ...
    <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      {step === 1 && (
        <>
          ...
          <div>
            <div style={{ fontFamily: 'Space Mono,monospace', fontSize: 9, color: theme.sub, letterSpacing: 2, marginBottom: 8 }}>
              AGENT NAME <span style={{ color: theme.dim }}>(optional)</span>
            </div>
            <input
              type="text"
              value={name}
        ...
        <>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {type === 'bills' ? (
              <>
                <div>
                  <div style={{ fontFamily: 'Space Mono,monospace', fontSize: 9, color: theme.sub, letterSpacing: 2, marginBottom: 8 }}>SELECT SERVICE</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {BILL_SERVICES.map(s => (
                      <button key={s} onClick={() => setService(s)} style={{
                        background: service === s ? `${theme.accent}18` : theme.card,
                        border: `1.5px solid ${service === s ? theme.accent : theme.bdr}`,
                        color: service === s ? theme.accent : theme.text,
                        padding: '8px 12px', fontFamily: 'Space Mono,monospace',
                        fontSize: 10, fontWeight: 700, cursor: 'pointer', borderRadius: 6,
                      }}>{s}</button>
                    ))}
                  </div>
                </div>
                <div>
                  <div style={{ fontFamily: 'Space Mono,monospace', fontSize: 9, color: theme.sub, letterSpacing: 2, marginBottom: 8 }}>MONTHLY BUDGET (TON)</div>
                  <div style={{ position: 'relative' }}>
                    <input type="number" value={amt} onChange={e => setAmt(e.target.value)} style={inputStyle} />
                    <span style={{ position: 'absolute', right: 14, top: '50%', transform: 'translateY(-50%)', fontFamily: 'Space Mono,monospace', fontSize: 11, color: theme.accent, fontWeight: 700 }}>TON</span>
                  </div>
                  <div style={{ fontFamily: 'Space Mono,monospace', fontSize: 10, color: theme.sub, marginTop: 5 }}>≈ ${usd} USD / month</div>
                </div>
              </>
            ) : (
              <>
        ...
        <>
          <div style={{
            ...
            <div style={{ background: `${theme.accent}10`, border: `1px solid ${theme.accent}33`, borderRadius: 6, padding: '12px', marginTop: 14 }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 800, color: theme.accent }}>{amt} {type === 'bills' ? 'TON/mo' : from}</div>
